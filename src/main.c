...
void createHalfAdder(Chips *chips, Vec2f position, u32 *carry_ptr) {
	...
	chips->array[carry].attachPosition = newVec2f(-55.0f, -20.0f);
	chips->array[carry].parentID = halfAdder;
	u32 sum = newSimpleChip(chips, XOR);
	chips->array[sum].attachPosition = newVec2f(0.0f, 340.0f);
...
void createFullAdder(Chips *chips, Vec2f position, u32 in1, u32 in2, u32 in3, u32 *carryOut) {
	u32 halfAdder = newChipEntity(chips, 0, CE_NONE);
	chips->array[halfAdder].position = position;

	u32 sum = newSimpleChip(chips, XOR);
	chips->array[sum].position = translateVec2f(position, newVec2f(55.0f, 260.0f));
	linkChipInsignal(chips, sum, 0, in1, 0);
	linkChipInsignal(chips, sum, 1, in2, 0);

	u32 carry1 = newSimpleChip(chips, AND);
	chips->array[carry1].position = translateVec2f(position, newVec2f(0.0f, -100.0f));
	linkChipInsignal(chips, carry1, 0, in1, 0);
	linkChipInsignal(chips, carry1, 1, in2, 0);

	u32 sum2 = newSimpleChip(chips, XOR);
	chips->array[sum2].position = translateVec2f(position, newVec2f(0.0f, 340.0f));
	linkChipInsignal(chips, sum2, 0, sum, 0);
	linkChipInsignal(chips, sum2, 1, in3, 0);

	u32 carry2 = newSimpleChip(chips, AND);
	chips->array[carry2].position = translateVec2f(position, newVec2f(-70.0f, -105.0f));
	linkChipInsignal(chips, carry2, 0, sum, 0);
	linkChipInsignal(chips, carry2, 1, in3, 0);

	*carryOut = newSimpleChip(chips, OR);
	chips->array[*carryOut].position = translateVec2f(position, newVec2f(-55.0f, -20.0f));
...
int main() {
	...
	chips->array[a].position = newVec2f(50.0f, 50.0f);
	u32 b = newInputChip(chips, SWITCH);
	chips->array[b].position = newVec2f(150.0f, 50.0f);
	u32 c = newInputChip(chips, SWITCH);
	chips->array[c].position = newVec2f(250.0f, 50.0f);

	u32 out = newSimpleChip(chips, XOR);
	chips->array[out].position = newVec2f(75.0f, 250.0f);
	linkChipInsignal(chips, out, 0, a, 0);
	linkChipInsignal(chips, out, 1, b, 0);

	u32 out2 = newSimpleChip(chips, AND);
	chips->array[out2].position = newVec2f(150.0f, 320.0f);
	...
	chips->array[s].position = newVec2f(350.0f, 50.0f);
	chips->array[r].position = newVec2f(450.0f, 50.0f);

	u32 nor = newSimpleChip(chips, NOR);
	chips->array[nor].position = newVec2f(350.0f, 250.0f);
	// in a real world, one gate would be reached faster
	getSimpleChip(chips, nor)->out = 1;

	u32 nor2 = newSimpleChip(chips, NOR);
	chips->array[nor2].position = newVec2f(450.0f, 200.0f);

	u32 orgate = newSimpleChip(chips, OR);
	chips->array[orgate].position = newVec2f(375.0f, 350.0f);
	...
	// half adder
	u32 carry1;
	createHalfAdder(chips, newVec2f(1200.0f, 550.0f), &carry1);

	// full adder
	u32 in1 = newInputChip(chips, SWITCH);
	chips->array[in1].position = newVec2f(1100.0f, 550.0f);
	u32 in2 = newInputChip(chips, SWITCH);
	chips->array[in2].position = newVec2f(1100.0f, 670.0f);
	u32 carry2;
	createFullAdder(chips, newVec2f(1100.0f, 550.0f), in1, in2, carry1, &carry2);

	u32 in3 = newInputChip(chips, SWITCH);
	chips->array[in3].position = newVec2f(1000.0f, 550.0f);
	u32 in4 = newInputChip(chips, SWITCH);
	chips->array[in4].position = newVec2f(1000.0f, 670.0f);
	u32 carry3;
	createFullAdder(chips, newVec2f(1000.0f, 550.0f), in3, in4, carry2, &carry3);

	u32 in5 = newInputChip(chips, SWITCH);
	chips->array[in5].position = newVec2f(900.0f, 550.0f);
	u32 in6 = newInputChip(chips, SWITCH);
	chips->array[in6].position = newVec2f(900.0f, 670.0f);
	u32 carry4;
	createFullAdder(chips, newVec2f(900.0f, 550.0f), in5, in6, carry3, &carry4);
